...
class _TasksPageState extends State<TasksPage>
  ...
  Future<List<Map<String, dynamic>>> _fetchAllTasks() async {
    try {
      ...
      final quizTasksWithAssignmentId =
          quizTasks.map((task) {
            ...
            return {
              ...
              'sort_date':
                  assignment?['assigned_date'] ?? task['tasks']?['created_at'],
            };
          }).toList();
      ...
      final essayTasks =
          (essayResponse as List).map((essay) {
            return {
              'id': essay['id'],
              'title': essay['title'],
              ...
              'sort_date':
                  essay['assignments']?['created_at'] ?? essay['created_at'],
            };
          }).toList();
  ...
  List<Map<String, dynamic>> _filterTasksByType(
    ...
    return allTasks
        .where((task) => task['assignment_type'] == filterType)
        .toList();
  }
      ...
      _quizChannel =
          supabase
              ...
              .onPostgresChanges(
                ...
                callback: (payload) {
                  debugPrint(
                    '📡 [REALTIME] Quiz deleted: ${payload.oldRecord}',
                  );
  ...
  void _toggleSortOrder() {
    setState(() {
      ...
      // Toggle order
      final newSortOrder =
          currentSortOrder == SortOrder.newestFirst
        ...
        Expanded(
          child: FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              final currentTabType = _taskTypes[_tabController.index];
              final filteredTasks = _filterTasksByType(
                allTasks,
                currentTabType,
              );
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return _buildTaskCard(item, isEssay, primaryColor, primaryLight);
        },
      ),
    );
  }
  ...
  Future<void> _viewQuiz(String quizId, String? quizTitle) async {
    try {
      // First try to get assignment ID and task ID
      String? assignmentId;
      String? taskId;
      ...
      try {
        ...
        taskId = assignmentResponse?['task_id'] as String?;
      } catch (e) {
          ...
          MaterialPageRoute(
            builder:
                (_) => QuizPreviewScreen(
                  ...
                  classRoomId: widget.classId, // Pass the classRoomId
                ),
          ),
        );
      }
  ...
  Future<void> _deleteEssay(Map<String, dynamic> item, String? title) async {
    ...
    try {
      ...
      if (storageFiles.isNotEmpty) {
        ...
        debugPrint(
          '✅ [STORAGE] Deleted $deletedCount/${storageFiles.length} files',
        );
      }
      ...
      // ========== NEW: Get student task progress records ==========
      ...
      if (taskId != null) {
        studentTaskProgress = await supabase
            ...
            .eq('task_id', taskId);
        debugPrint(
          '  Found ${studentTaskProgress.length} student task progress records',
        );
      }
      ...
      // d. Delete student task progress (depend on task) - MUST BE BEFORE TASK
      ...
      // e. Delete task materials (depend on task) - MUST BE BEFORE TASK
      ...
      // g. Delete the task (depend on nothing else now)
      ...
      // h. Finally, delete the main assignment
          ...
          const SnackBar(
            content: Row(
              children: [
                ...
                SizedBox(width: 8),
                Text(
                  'Essay assignment and all related data deleted successfully',
                ),
              ],
            ),
            backgroundColor: Colors.green,
  ...
  Future<void> _deleteQuiz(Map<String, dynamic> item, String? quizTitle) async {
    ...
    try {
      ...
      if (questionIds.isNotEmpty) {
        ...
        optionIds =
            (options as List).map<String>((o) => o['id'] as String).toList();
      ...
      if (questionIds.isNotEmpty) {
        ...
        recordingIds =
            (recordings as List).map<String>((r) => r['id'] as String).toList();
          ...
          const SnackBar(
            content: Row(
              children: [
                ...
                SizedBox(width: 8),
                Text(
                  'Quiz assignment and all related data deleted successfully',
                ),
              ],
            ),
            backgroundColor: Colors.green,
